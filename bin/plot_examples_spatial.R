...
gene_descr <- readRDS("data/gene.descr.rds")
...
vs <- lapply(sids, function(s) {
  v <- visutils::myLoad10X_Spatial(paste0(path2cr, "/", s, "/outs"), filter.matrix = TRUE, ens_id = TRUE, slice = s)
...
for (s in names(vs)) {
  visutils::plotVisium(vs[[s]], psi[colnames(vs[[s]])], main = s, legend.args = list(title = "PSI"), zlim = c(0, 0.7), cex = 0.)
}
...
for (s in names(vs)) {
  ...
  visutils::plotVisium(vs[[s]], cpm, main = s, zfun = log1p, legend.args = list(title = "CP10K"))
}
